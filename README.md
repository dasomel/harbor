...
- **`harbor-exporter` is `amd64`-only on tagged releases (`v2.15.x`).** Upstream Harbor has never shipped an ARM64-capable exporter: its `Dockerfile` self-compiles the binary in-image with `ENV GOARCH=amd64` **hardcoded since the exporter was introduced in `v2.2.0`**, and upstream published no multi-arch images for any release (`git ls-tree` shows zero `Dockerfile.multiarch` files in `v2.2.0`…`v2.15.2`). The upstream fix — [goharbor/harbor#22311](https://github.com/goharbor/harbor/pull/22311) "Full Multi-Architecture Enablement", which changes `GOARCH=amd64` → `GOARCH=${TARGETARCH}` — currently lives only on upstream `master` and is **not part of any `v2.15.x` release**. Because this fork builds each release from its exact upstream source, the exporter is **skipped** for tagged builds rather than published as a mislabeled amd64-only image in an ARM64 manifest slot. A genuinely multi-arch `harbor-exporter` is therefore available **only via the `:latest` tag** (built from this fork's `main`, which carries a fork-added multi-arch `Dockerfile`). This resolves automatically once upstream ships a release containing #22311 (expected in the `v2.16` line).
