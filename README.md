...
- **`harbor-exporter` is `amd64`-only on tagged releases (`v2.15.x`).** Upstream Harbor has never shipped an ARM64-capable exporter: its `Dockerfile` self-compiles the binary in-image with `ENV GOARCH=amd64` **hardcoded since the exporter was introduced in `v2.2.0`**, and upstream published no multi-arch images for any release (`git ls-tree` shows zero `Dockerfile.multiarch` files in `v2.2.0`…`v2.15.2`). The upstream fix — [goharbor/harbor#22311](https://github.com/goharbor/harbor/pull/22311) "Full Multi-Architecture Enablement", which changes `GOARCH=amd64` → `GOARCH=${TARGETARCH}` — was **merged to upstream `main` on 2026-05-12 and is already synced into this fork's `main`**, but is **not part of any `v2.15.x` release** (all `v2.15.x` tags predate it). Because this fork builds each release from its exact upstream source, the exporter is **skipped** for those tagged builds rather than published as a mislabeled amd64-only image in an ARM64 manifest slot. A genuinely multi-arch `harbor-exporter` is therefore available **only via the `:latest` tag** (built from this fork's `main`, which now carries upstream's own multi-arch exporter `Dockerfile`). This resolves for tagged builds automatically once upstream ships a release containing #22311 (expected in the `v2.16` line) — at which point the exporter no longer needs to be skipped.
- **`:latest` tracks the newest completed build, not the highest semver.** The pipeline pushes both `:<version>` and `:latest` on every build, so `:latest` always reflects whichever release was built most recently — which, if releases are (re)built out of order, may not be the highest version number.
...
This fork is distributed under the same [Apache License 2.0](LICENSE) as the original Harbor project.
All original copyrights and attributions are preserved.
